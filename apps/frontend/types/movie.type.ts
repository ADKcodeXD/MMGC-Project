...
export type MovieParams = Pick<
  MovieModel,
  'movieCover' | 'movieName' | 'movieDesc' | 'moviePlaylink'
> &
  Partial<
    Pick<
      MovieModel,
...
export interface MovieUpdateParams extends MovieParams {
  movieId: number
}
export interface MovieVo
  extends Required<Omit<MovieModel, 'activityId' | 'authorId' | 'uploader' | 'expectPlayTime'>> {
  /**
   * 活动VO类型
   */
